...
Each pose of `POSES` (SPACE for the next); **green** sphere at MuJoCo's foot
site, **red** at the file's own forward kinematics. **Both FKs are complete and
explained** — `chain_fk` (six bodies, 1.8e-16) and `paper_fk` (three angles,
2.2e-06) — and `F` switches them. Nothing is simulated.
...
## Lab class: `fk.py`, the chain one body at a time {.smaller}

```bash
uv run weeks/02-transforms/fk.py --angles -0.35 0 0 0.70 -0.35 0
```

Places the leg, walks pelvis → six bodies → foot site by hand, and prints a
line per body: the offset from its parent, the joint, the angle, and where
that leaves us. Then the paper model beside it, then the pose in the simulator.
...
Calls, in order: `leg_qpos_indices`, `mj_forward`, `leg_chain`, `body_pos`,
`body_quat`, `jnt_axis`, `mju_quat2Mat`, `mju_axisAngle2Quat`, `site_pos`,
`site_xpos`; then `launch_viewer(passive=True)` with two `user_scn` spheres.
