...
`G1WalkEnv` under a PPO policy from `CONFIGS`; `T` trains it in the background
(70 s for 30k steps) while the window keeps playing; `D` switches the mean
action for a sampled one; `ENTER` prints the action std. **The code is
complete**: `choose_action` is PPO's own sampling, explained.
...
## Lab class: `train.py`, two robots measured {.smaller}
...
Evaluates the untrained policy both ways, trains with a progress line every
5000 steps, evaluates both ways again, replays the trained mean robot.
