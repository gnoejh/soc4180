...
**Ten problems, one score out of 10.** Wave, sway, twist, squat, bow, clap,
both hands up, squat-wave, twist-wave, and a boss. You edit **only
`moves.py`**: each answer is a *move*, a list of `(seconds, pose)` steps the
G1 blends through **under real physics**.

A pose names joints in degrees, or uses this lecture's solver:
`"pelvis": [0, 0, -0.2]` lowers the body 20 cm and `ik_legs` finds the twelve
leg angles that keep both feet planted.

| Key | Does |
| --- | --- |
| `1` … `9`, `0` | play a problem, with a live gauge ("left hand 1.12 m, need > 1.20") |
| `E`, then `K` / `ENTER` | freeze and pose by slider; step through your move; print the pose to paste |
| `G` | grade all ten; the score and a 4-letter scorer code stay on screen |

`moves.py` ships with its key numbers left as `...` — fill them in. It is
re-read on every key press: save, press again, no restart.

---

## Lab class: what the game teaches {.smaller}

Every pose in the game is one IK solves perfectly — frozen. **Under gravity,
half of them fall over.** Measured with the game's own `--grade`:

| Problem | The naive answer | What works |
| --- | --- | --- |
| Hands up | both arms at once: falls on its face | one arm, then the other: 1.36 m |
| Clap | arms forward: falls forward | pelvis 4 cm back as a counterweight |
| Bow | hips −30°, waist +30°: topples | push the pelvis back and down while bending |
| Boss | lower the arms, then stand: falls on its back | stand up *first*, arms still up |

**Kinematics places a pose; it does not balance a robot.** The order of the
poses is part of the answer. That gap is weeks 4 and 5.

A pass needs three things: the goal reached, no fall on the way, and the robot
ends standing. Take as long as you like — slow moves often balance.
