...
Written as equations, one loop is three functions, and each is a layer:

$$x_{k+1} = f(x_k, u_k)\qquad y_k = h(x_k)\qquad u_k = \pi(y_k)$$

$f$ is the world (Layer 1), $h$ is what the sensors declared in the model can
see (Layer 2), and $\pi$ is everything that decides (Layers 4 and 5). The
whole semester is about $\pi$; the first half writes it by hand, the second
half learns it.
...
$$\tau = k_p\,(\text{ctrl} - q) \;-\; k_v\,\dot q, \qquad k_p = 500$$

A spring pulling the joint toward its setpoint, plus a damper. Week 5 takes
this apart; today it is enough to know that a *command* is an angle, and a
*torque* is what the spring produces when the joint is not there yet.
...
## The same numbers, over time {.smaller}
...
The blue line is flat to the width of the line. The orange one takes about
half a second to reach the floor and then bounces in the heap. **Nothing about
the robot differs between the two lines.**
...
## The ladder, on a log axis {.smaller}
...
Our policy will run at 50 Hz over 500 Hz physics: **ten physics steps per
decision**, and the servos correct the joints ten times between two thoughts.
...
The G1 in the interactive viewer, with each key leaving a different set of
layers switched on: `1` physics and model only (rag doll), `0` the `ctrl = 0`
trap, `2` servos holding `stand`, `3` the week 4 walker, `4` your own
`my_controller`, `G` Moon gravity with nothing else touched. Contact forces are
drawn as arrows on the floor — Layer 1, visible.

| Step | Do | Right looks like |
| --- | --- | --- |
| 1 | `1`, `R`, `2` | one sentence on what differs, and which layer it is |
| 2 | `R`, `0` | a standing robot, and why zero is a command |
| 3 | `G`, `R`, `1`; then `3` | a slow fall; a walker that does not know it is on the Moon |
| 4 | write `my_controller`: wave the left arm | the arm waves, the robot stands; the student says which layer keeps it up |
| 5 | `ENTER` standing, then walking | forces sum to the weight; then they do not, and why |
