...
controller driving all of them — walking side by side in the viewer. `+`/`-`
add and remove robots, `F` randomises each one's friction and mass (they
drift apart within steps: week 11 in one key), `P` benchmarks 1, 2, 4 and 8
processes, and `ENTER` prints robot-steps per second against the student's
`predict_rate`.
