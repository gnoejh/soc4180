...
**Forward kinematics, in velocities** (last week's Jacobian):

$$\dot{\mathbf{x}} = J(q)\,\dot{q}$$

- $q$ — the vector of joint angles; $\dot{q}$ — how fast each joint is turning
- $\mathbf{x}$ — the foot (end-effector) position and orientation; $\dot{\mathbf{x}}$ — how fast the foot moves
- $J(q)$ — the Jacobian: the matrix that turns joint speeds into foot speed, and changes with the pose

*If you know how fast the joints move, the Jacobian tells you how fast the foot moves.*

**Inverse kinematics asks the opposite:** *what joint motion produces the foot
motion I want?* So do not solve for the angles. Solve for a small **change** in
the angles that moves the foot toward the target, then repeat. Rearranged:

$$J\,\Delta q = \mathbf{e}$$

- $\mathbf{e}$ — the desired foot motion: a small step toward the target
- $\Delta q$ — the joint-angle change that produces that motion
...
## The obvious answer: $\Delta q = J^{-1}\mathbf{e}$ {.smaller}

Six equations, six unknowns: for one leg $J$ is $6 \times 6$, so if it is
invertible the system $J\,\Delta q = \mathbf{e}$ has exactly one solution,

$$\Delta q = J^{-1}\,\mathbf{e}.$$

$J$ maps joint motion to foot motion; $J^{-1}$ runs that map backwards. Hand it
the foot motion you want and it returns the joint motion that produces it —
exactly, so that $J\,\Delta q = \mathbf{e}$ holds to machine precision.
...
the foot can move is **divided by its own $\sigma_i$**. A direction the leg
barely moves (small $\sigma_i$) needs $1/\sigma_i$ of joint motion; at a
singularity $\sigma_i = 0$ and no solution exists — and just beside it, a huge one.
...
Instead of *solving* $J\,\Delta q = \mathbf{e}$, ask for the $\Delta q$ that
**minimises a cost** with two terms — the error left over, and the joint motion
spent:

$$C(\Delta q) = \|J\,\Delta q - \mathbf{e}\|^2 + \lambda^2\,\|\Delta q\|^2 .$$

**Step 1 — expand.** Using $\|v\|^2 = v^{\mathsf T}v$:

$$C = \Delta q^{\mathsf T} J^{\mathsf T} J\,\Delta q
    \;-\; 2\,\mathbf{e}^{\mathsf T} J\,\Delta q
    \;+\; \mathbf{e}^{\mathsf T}\mathbf{e}
    \;+\; \lambda^2\,\Delta q^{\mathsf T}\Delta q .$$

**Step 2 — differentiate with respect to $\Delta q$** (for a symmetric $A$,
$\nabla\, v^{\mathsf T} A v = 2Av$ and $\nabla\, b^{\mathsf T} v = b$):

$$\nabla C = 2\,J^{\mathsf T} J\,\Delta q \;-\; 2\,J^{\mathsf T}\mathbf{e} \;+\; 2\lambda^2\,\Delta q .$$

**Step 3 — set it to zero.** The minimum satisfies the *normal equations*

$$\left(J^{\mathsf T} J + \lambda^2 I\right)\Delta q = J^{\mathsf T}\mathbf{e}
\qquad\Longrightarrow\qquad
\Delta q = \left(J^{\mathsf T} J + \lambda^2 I\right)^{-1} J^{\mathsf T}\mathbf{e}.$$

It *is* the minimum, not just a stationary point: the Hessian $2(J^{\mathsf T}J +
\lambda^2 I)$ is positive definite for any $\lambda > 0$, because
$v^{\mathsf T}J^{\mathsf T}Jv = \|Jv\|^2 \ge 0$ and $\lambda^2\|v\|^2 > 0$. The
same fact says the inverse **always exists** — even at a singularity, where
$J^{-1}$ does not. That is the whole point of the second term.

---

## Where the formula comes from: two forms, one answer {.smaller}
...
$$\Delta q = V\,\underbrace{(\Sigma^2 + \lambda^2 I)^{-1}\Sigma}_{\text{diagonal}}\;U^{\mathsf T}\mathbf{e}:
\qquad \text{direction } i \text{ is scaled by } \frac{\sigma_i}{\sigma_i^2 + \lambda^2}
\approx \begin{cases} 1/\sigma_i & \sigma_i \gg \lambda \quad\text{the true inverse}\\[2pt]
